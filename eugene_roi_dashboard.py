...
# ✅ Initialize session state variables to prevent KeyErrors
if "input_mode_selection" not in st.session_state:
    st.session_state["input_mode_selection"] = "Simplified"  # Default input mode

if "user_type" not in st.session_state:
    st.session_state["user_type"] = "Doctor/Clinician"  # Default user type

if "results" not in st.session_state:
    st.session_state["results"] = {}  # Ensure results storage is available

# ✅ Debugging (Optional: Print session state for verification)
...
def display_logo(logo_base64):
    """Displays a base64 encoded logo in Streamlit."""
    if logo_base64.strip():  # Ensure it's not empty
...
def get_user_type():
    ...
    return st.radio(
        ...
        horizontal=True,
        key="user_type"  # Streamlit manages it, preventing errors
    )
...
def get_practice_profile():
    ...
    with st.expander("🏥 Practice Profile", expanded=True):
        cols = st.columns(2)
        practice = {
            "specialty": cols[0].selectbox("Medical Specialty", list(SPECIALTY_MBS.keys())),
        ...
        # If Owner/Manager, ask for number of doctors
...
def get_staff_costs():
    """Collects staff costs dynamically."""
    with st.expander("💰 Staff Costs", expanded=True):
        ...
        # Salary values change based on mode
        if st.session_state["input_mode_selection"] == "Advanced":
            ...
            specialty = st.session_state["user_type"]
            staff["doctor_hourly"] = SPECIALTY_MBS[specialty]["Core"]["rate"]

        return staff

def get_logistical_costs():
    """Collects logistical costs (Only if Fertility Clinic)."""
...
def get_billing_model():
    ...
    with st.expander("💵 Billing Model", expanded=True):
        ...
        if model == "Mixed":
            ...
            config["bulk_rate"] = min(bulk_rate, 100)  # Ensure ≤ 100%
        
        return config

def get_test_configuration(test_category):
    """Collects test parameters dynamically based on test type."""
    ...
    with st.expander(f"🧬 {test_category} Testing"):
        return {
            ...
            curly: {
                "weekly_volume": st.number_input("Complex Cases/Week", 0, 500, 10, key=f"vol_{test_category}_complex"),
...
def calculate_logistical_costs(logistics):
    ...
    try:
        return sum([v for v in logistics.values()]) * 12 if logistics else 0  # Convert monthly to yearly
    except Exception as e:
        st.error(f"Error in logistics calculation: {str(e)}")
        return 0

def calculate_efficiency_savings(test_config, staff, weeks_year):
    """Calculates efficiency savings for each role and test type."""
    savings = {}
    ...
    try:
        for test_type, test_variants in test_config.items():
            for variant, params in test_variants.items():
                ...
                role_savings = {
                    role: (time_components[role] * annual_volume * staff[f"{role}_hourly"])
                    for role in ["admin", "nurse", "doctor", "genetic"] if f"{role}_hourly" in staff
                }
        ...
        for test_type, test_variants in test_configs.items():
            for variant, params in test_variants.items():
                ...
                if billing_model["model"] == "Bulk Bill":
                    ...
                    total_revenue += (bulk_volume * rate) + (private_volume * billing_model["private_hourly"] * (params.get("doctor_time", 0) / 60))
...
def main():
    ...
    # ✅ Ensure session state for results
    if "results" not in st.session_state:
        st.session_state["results"] = {}

    # Create Two-Column Layout (Inputs on Left, Financial Summary + Graphs on Right)
    ...
    with col_input:
        ...
        user_type = st.radio("Select your role:", ["Doctor/Clinician", "Owner/Manager"], horizontal=True, key="user_role")

        # Step 2: Input Mode Selection (Simplified or Advanced)
        input_mode = st.radio("Select Input Mode:", ["Simplified", "Advanced"], horizontal=True, key="input_mode_selection")

        # Step 3: Practice Profile Input
        practice = get_practice_profile()

        # Step 4: Staff Cost Inputs (Only for Owners)
        if user_type == "Owner/Manager":
            staff = get_staff_costs()
        else:
            # Default single doctor setup for Clinicians
            staff = {
                "num_admin": 1,
                "num_nurse": 1,
                "num_doctor": 1,
                "admin_hourly": 25,
                "nurse_hourly": 50,
                "doctor_hourly": SPECIALTY_MBS[practice["specialty"]]["Core"]["rate"]
            }

        # Step 5: Logistical Costs (Only for Owners)
        logistics = get_logistical_costs() if user_type == "Owner/Manager" else {}

        # Step 6: Billing Model Selection
    ...
    with col_output:
        if calculate:
            ...
            # 🔹 Fix: Ensure Revenue Breakdown is stored correctly
            revenue_data = []
            for test_type, test_variants in test_configs.items():
                for variant, params in test_variants.items():
            ...
            st.session_state["results"]["Revenue Breakdown"] = df_revenue  # ✅ Store Revenue Breakdown
...
def export_to_excel():
    ...
    results = st.session_state["results"]  # Retrieve stored results
    
    # ✅ Ensure Revenue Breakdown exists before exporting
