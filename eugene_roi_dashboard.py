...
def get_staff_costs():
    """Collects staff costs dynamically and uses default fallback in simplified mode."""
...
def get_test_configuration(test_category):
    """Collects test parameters dynamically based on input mode."""
    ...
    with st.expander(f"🧬 {test_category} Testing"):
        if st.session_state["input_mode_selection"] == "Advanced":
            ...
            # Simplified: Only ask for weekly volume; times come from assumptions
            weekly_volume = st.number_input(f"{base} Tests/Week (Simplified)", 0, 1000, 20, key=f"vol_{test_category}_base_simple")
            complex_cases = weekly_volume * 0.3  # Assume 30% are complex by default
            simplified_times = SIMPLIFIED_TIME[test_category]
            return {
                base: {
                    "weekly_volume": weekly_volume,
                    "admin_time": simplified_times["admin"],
                    "nurse_time": simplified_times["nurse"],
                    "doctor_time": simplified_times["doctor"],
                },
                curly: {
                    "weekly_volume": complex_cases,
                    "research_time": simplified_times["research"],
                    "genetic_time": 60,  # Literature-based assumption
                }
            }
...
def main():
    ...
    with col_input:
        ...
        # ✅ Use helper functions for consistent session state handling
        ...
        if user_type == "Owner/Manager":
            ...
            # Doctor/Clinician default setup for Simplified mode
            specialty = st.session_state.get("specialty", "GP")
            staff = {
                "num_admin": 1,
                "num_nurse": 1,
